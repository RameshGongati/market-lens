...
def _support_resistance(data: pd.DataFrame) -> tuple[float, float]:
    ...
    highs = data["High"]
    lows = data["Low"]
    resistance = float(highs.rolling(5).max().iloc[-1])
    support = float(lows.rolling(5).min().iloc[-1])
...
class IntradayAnalysis(BaseAnalysis):
    ...
    def analyse(self, symbol: str, data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            # Downgrade if RSI is extreme counter to trend
            ...
            summary = (
                ...
                f"RSI {rsi:.0f} | Vol {volume_ratio:.1f}x avg"
            )
