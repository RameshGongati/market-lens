...
class LongTermAnalysis(BaseAnalysis):
    ...
    def analyse(self, symbol: str, data: pd.DataFrame) -> dict[str, Any]:
        ...
        Args:
            ...
            data: OHLCV DataFrame — recommend at least 200 daily bars.

        Returns:
            Dict with: symbol, current_price, sma_200, trend, high_52w,
            low_52w, position_pct, recommendation, status, summary.
        """
        ...
        try:
            ...
            # Use last 252 trading days (≈1 year) for 52-week range
            ...
            recommendation = _build_recommendation(
                symbol, current_price, sma_200, trend, position_pct
            )
            summary = f"{symbol}: {trend.capitalize()} | {position_pct:.0f}% of 52w range | SMA200: ₹{sma_200:.2f}"
...
def _build_recommendation(
    ...
    if trend == "uptrend":
        ...
        if position_pct > 80:
            lines.append("Caution: Price is near its 52-week high — consider waiting for a pullback before adding positions.")
        else:
            lines.append("Price has room to run within the 52-week range. Suitable for long-term accumulation on dips.")
        ...
        if position_pct < 20:
            lines.append("Price is near its 52-week low — watch for base formation before considering entry.")
    else:
        lines.append(
